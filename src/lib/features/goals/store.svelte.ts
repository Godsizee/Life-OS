...
import type { Goal, GoalStatus, JournalEntry } from './types';
...
class GoalsState {
	...
	async saveJournalEntry(date: string, mood: string | null, body: string) {
		if (!this.workspaceId) throw new Error('Kein Workspace geladen');
	...
	async saveTodayEntry(mood: string | null, body: string) {
		await this.saveJournalEntry(this.todayKey, mood, body);
	}
