...
class HabitsState {
	...
	async updateGoalLink(id: string, goal_id: string | null) {
		...
		this.habits = this.habits.map((h) => (h.id === id ? { ...h, goal_id, updated_at } : h));
		await outbox.runOrQueue('habits', 'update', { id, goal_id, updated_at }, () =>
			habitsApi.updateRaw({ id, goal_id, updated_at })
		);
	}
