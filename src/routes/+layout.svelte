<script lang="ts">
	...
	const syncBanner = $derived(
		!online
			? { text: 'Offline – Änderungen werden gespeichert', class: 'bg-slate-700' }
			: outbox.status === 'syncing'
				? { text: 'Synchronisiere…', class: 'bg-emerald-600' }
				: outbox.status === 'error'
...
<div class="flex min-h-screen flex-col bg-slate-50 text-slate-900 {showNav ? 'pb-16' : ''}">
	{#if syncBanner}
		<button
